 ...
 * @Last Modified time: 2019-04-25 16:38:02
 * @Init Method
 */
...
var insertMasterDocDataToEs = function (mongodataCount, currentNum, file) {
    return new Promise(function (resolve, reject) {
        ...
        return Promise.delay(delayTime).then(function () {
        ...
        creatTail(file, filePath).then(function (result) {
            if (result > 0) {
                ...
                return Promise.each(currentNumArray, function (currentNum) {
                    ...
                    if (result) {
                        var refresh_interval = "30s";
                        var replicas = 1;
                        if (file.Content.elasticsearch.refresh_interval) {
                            refresh_interval = file.Content.elasticsearch.refresh_interval;
                        }
                        if (file.Content.elasticsearch.replicas) {
                            replicas = file.Content.elasticsearch.replicas;
                        }
                        var putSettingsObj = {
