 ...
 * @Last Modified time: 2018-09-11 14:54:26
 * @Init Method
 */
    ...
    return new Promise(function (resolve, reject) {
        ...
        setTimeout(function () {
            ...
            if (file.Content.mongodb.m_extendinit) {
                filterfilds = util.mergeJsonObject(file.Content.mongodb.m_filterfilds, util.extendinit(file.Content.mongodb.m_extendinit));
            } else {
            ...
            return new Promise(function (resolve, reject) {
                ...
                singleMasterDoc(item, filePath).then(function (result) {
                    if (result) {
                        ...
                        tail.tail(item.Content, filePath);
                        return resolve(result);
                    } else {
                        logger.errMethod(item.Content.elasticsearch.e_connection.e_server, item.Content.elasticsearch.e_index,
                            "DataConfig Master document has some problems, FileName is : " + item.Filename);
                        return resolve(false);
                    }
                });
            });
