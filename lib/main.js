        ...
        elasticsearchPromise.existEsServer(file.Content.elasticsearch.e_connection.e_server,
            file.Content.elasticsearch.e_connection.e_httpauth).then(function (result) {
                if (result) {
                    ...
                    logger.error('Unable to revive connection: %s', file.Content.elasticsearch.e_connection.e_server);
                    return 0;
                }
                ...
                if (result > 0) {
                    ...
                    if (file.Content.mongodb.m_documentsinbatch > 0) {
                        totalNum = Math.ceil(result / file.Content.mongodb.m_documentsinbatch);
                        for (var i = 0; i < totalNum; i++) {
                            promiseArray.push(insertDataToEs(i, file));
                        }
                    }
                    else {
                        promiseArray.push(insertDataToEs(0, file));
                    }
...
var insertDataToEs = function (currentNum, file) {
    return new Promise(function (resolve, reject) {
        mongoPromise.getPageDataResult(util.returnMongodbDataUrl(file.Content.mongodb.m_connection,
            file.Content.mongodb.m_database), file.Content.mongodb.m_collectionname, file.Content.mongodb.m_filterfilds,
...
var getDataArrayAsnyc = function (results, file) {
    return new Promise(function (resolve, reject) {
        var mongoResults = [];
        if (results.length > 0) {
            Promise.reduce(results, (total, item, index) => {
                return new Promise(function (resolve, reject) {
