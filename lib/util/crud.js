...
var insert = function (index, type, document_id, document, url) {
...
var remove = function (index, type, document_id, url) {
  logger.debug(chalk.bgHex('#EE30A7').bold('Deleting document %s'), document_id);
  return new Promise(function (resolve, reject) {
    elasticsearchPool.getConnection(url).then(function (client) {
      client.delete({
        index: index,
...
var update = function (index, type, document_id, partialDocument, url) {
  logger.debug(chalk.hex('#FF34B3').bgHex('#FFFF00').bold('Updating document %s'), document_id);
  return new Promise(function (resolve, reject) {
    elasticsearchPool.getConnection(url).then(function (client) {
      client.update({
        index: index,
