 ...
 * @Last Modified time: 2019-05-21 19:18:37
 * @Common Method
 */
...
var logPath = path.join(appRoot.path, 'logs');
            ...
            files.forEach(function (filename) {
                ...
                if (stat.isDirectory()) {
                    ...
                    if (path.extname(filename) === extName) {
                        var currentFile = {};
                        var currentFileContent = fs.readFileSync(path.join(filePath, filename), "utf-8");
                        currentFile.Filename = filename;
                        currentFile.Content = JSON.parse(ase.aesDecrypt(currentFileContent, "pwd"));
...
var findInfoArrayStatus = function (cluster, index, ) {
    var status;
    if (global.infoArray && global.infoArray.length > 0) {
        _.find(global.infoArray, function (file) {
            if (file.cluster === cluster && file.index === index) {
...
module.exports = {
    ...
    findInfoArrayStatus: findInfoArrayStatus,
