...
var returnMongodbDataUrl = function (connection, dataBase) {
    ...
    if (connection.mongodb_servers && connection.mongodb_servers.length > 0) {
        if (connection.mongodb_authentication) {
            if (connection.mongodb_authentication.userName && connection.mongodb_authentication.passWord && connection.mongodb_authentication.authSource && connection.mongodb_authentication.replicaSet) {
                mongodbDataUrl = "mongodb://" + connection.mongodb_authentication.userName + ":" + connection.mongodb_authentication.passWord
                    + "@" + connection.mongodb_servers.toString() + "/" + dataBase + "?authSource=" + connection.mongodb_authentication.authSource + "&replicaSet=" + connection.mongodb_authentication.replicaSet;
            }
        }
        else {
...
var returnMongodbOplogUrl = function (connection) {
    ...
    if (connection.mongodb_servers && connection.mongodb_servers.length > 0) {
        if (connection.mongodb_authentication) {
            if (connection.mongodb_authentication.userName && connection.mongodb_authentication.passWord && connection.mongodb_authentication.authSource && connection.mongodb_authentication.replicaSet) {
                mongodbOplogUrl = "mongodb://" + connection.mongodb_authentication.userName + ":" + connection.mongodb_authentication.passWord
                    + "@" + connection.mongodb_servers.toString() + "/local" + "?authSource=" + connection.mongodb_authentication.authSource + "&replicaSet=" + connection.mongodb_authentication.replicaSet;
            }
        }
        else {
