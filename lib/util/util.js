...
module.exports = {
  ...
  transform: function (watcher, document, callBack) {
    logger.verbose('Transform','Transforming document: %s', JSON.stringify(document).trunc(200));
    if (document.index) {
      callBack(document);
    } else {
      ...
      logger.verbose('Transform','Transform complete: %s', JSON.stringify(document).trunc(200));
      callBack(document);
