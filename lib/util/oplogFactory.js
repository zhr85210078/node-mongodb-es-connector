                ...
                return new Promise(function (resolve, reject) {
                    if (opType == "delete") {
                        return deleteDoc(watcher.Content.elasticsearch.e_connection.e_server, watcher.Content.elasticsearch.e_connection.e_httpauth,
                            obj, watcher.Content.elasticsearch.e_index, watcher.Content.elasticsearch.e_type, opType).then(function (res) {
                            return resolve(res);
                        });
                    }
                    var opDoc = {};
                    ...
                    if (Util.filterJson(watcher.Content.mongodb.m_filterfilds, obj)) {
                        ...
                        if (watcher.Content.elasticsearch.e_pipeline && watcher.Content.elasticsearch.e_iscontainattachment) {
                            ...
                            return pipelineAndAttachmentsBulk(mongodbUrl, watcher.Content.elasticsearch.e_connection.e_server,
                                ...
                                watcher.Content.elasticsearch.e_type, id, watcher.Content.elasticsearch.e_pipeline, opType).then(function (res) {
                                return resolve(res);
                            });
                            ...
                            return pipelineBulk(watcher.Content.elasticsearch.e_connection.e_server, watcher.Content.elasticsearch.e_connection.e_httpauth,
                                bulk, watcher.Content.elasticsearch.e_index, watcher.Content.elasticsearch.e_type, id, watcher.Content.elasticsearch.e_pipeline, opType).then(function (res) {
                                return resolve(res);
                            });
                        } else {
                            return normalBulk(watcher.Content.elasticsearch.e_connection.e_server, watcher.Content.elasticsearch.e_connection.e_httpauth,
                                bulk, watcher.Content.elasticsearch.e_index, watcher.Content.elasticsearch.e_type, id, opType).then(function (res) {
                                return resolve(res);
                            });
                        }
                    } else {
...
var oplogInit = function (filePath, currentWatcher, doc, opType) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        if (collectionName == "files") {
            opType = "update";
            if (doc.o.metadata) {
                var mongoDataUrl = Util.returnMongodbDataUrl(currentWatcher.mongodb.m_url, currentWatcher.mongodb.m_connection,
                ...
                return mongoPromise.getOneData(mongoDataUrl, currentWatcher.mongodb.m_collectionname, doc.o.metadata.MainId).then(function (result) {
                    return dataFactory(filePath, mServer, database, currentWatcher.mongodb.m_collectionname, result, opType).then(function (res) {
                        return resolve(res);
                    });
                });
            }
        } else {
            return dataFactory(filePath, mServer, database, collectionName, doc.o, opType).then(function (res) {
                return resolve(res);
            });
        }
    }).then(function (res) {
