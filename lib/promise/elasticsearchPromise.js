        ...
        elasticsearchPool.getConnection(url, esHttpAuth).then(function (client) {
            client.search({
                ...
                if (err) {
                    ...
                    logger.error('queryByIndex errors: %s', response.errors);
                    return resolve(hits);
                }
                else {
        ...
        elasticsearchPool.getConnection(url, esHttpAuth).then(function (client) {
            client.bulk({
                ...
                if (err) {
                    ...
                    logger.error('bulkData errors: %s', response.errors);
                    return resolve(true);
                }
                else {
                    return resolve(true);
        ...
        elasticsearchPool.getConnection(url, esHttpAuth).then(function (client) {
            client.bulk({
                ...
                if (err) {
                    ...
                    logger.error('bulkDataAndPip errors: %s', response.errors);
                    return resolve(true);
                }
                else {
                    return resolve(true);
        ...
        elasticsearchPool.getConnection(url, esHttpAuth).then(function (client) {
            client.indices.delete({
                ...
                if (err) {
                    ...
                    logger.error('deleteByIndex errors: %s', response.errors);
                    return resolve(true);
                }
                else {
                    return resolve(true);
        ...
        elasticsearchPool.getConnection(url, esHttpAuth).then(function (client) {
            client.delete({
                ...
                if (err) {
                    ...
                    logger.error('removeDoc errors: %s', response.errors);
                    return resolve(true);
                }
                else {
                    return resolve(true);
        ...
        elasticsearchPool.getConnection(url, esHttpAuth).then(function (client) {
            client.search({
                ...
                if (error) {
                    ...
                    logger.error('existDoc errors: %s', response.errors);
                    return resolve(true);
                }
                else {
                    var flag = false;
