 ...
 * @Last Modified time: 2019-04-29 17:21:30
 * @Mogodb Method
 */
...
var getGridFsArray = function (mongoUrl, mainId, attachmentSize) {
    return new Promise(function (resolve, reject) {
        ...
        getGridFsFileByMetaDataMainId(mongoUrl, mainId).then(function (files) {
            ...
            if (files.length > 0) {
                Promise.reduce(files, function (total, item) {
                    return new Promise(function (resolve, reject) {
                        if (!attachmentSize || item.length < maxAttachmentSize) {
                            ...
                            resultArry[0] = "Exceed max length: fileId(" + item._id.toString() + ")";
                            return resolve(resultArry);
                        }
                    });
                }, 0).then(function (res) {
                    if (attachmentSize && attachments_SizeByte > maxAttachmentSize) {
                        resultArry = [];
                        resultArry[0] = "Exceed max length: mainId(" + mainId + ")";
                        return resolve(resultArry);
                    }
                    return resolve(res);
                }).catch(function (err) {
                ...
                resultArry[0] = "no attachments found";
                return resolve(resultArry);
            }
        });
    });
